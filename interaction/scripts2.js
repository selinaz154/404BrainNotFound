document.addEventListener("DOMContentLoaded", function () {
  ...
  async function loadDefaultGraph() {
    const jsonData = await fetchJSON("https://selinaz154.github.io/404BrainNotFound/new4.json");
  ...
  async function processFormData() {
    ...
    const filteredData = jsonData.filter((d) => {
      ...
      if (monthDifference < 0 || (monthDifference === 0 && performedDate.getDate() < birthDate.getDate())) {
        ageAtProcedure--;
      }
      ...
      const matchesMaritalStatus = married === "all" || d.MaritalStatus === married;
